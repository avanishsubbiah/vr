...
foreach ($mon_info in $monitor_info_arr) {
    ...
    if (([int]$width -gt $min_width) -And ([int]$height -gt $min_height)) {
        if ([int]$refresh -gt $min_refesh) { # Refresh rate greater than minimum
            $set_VR_res_cmd_arr.Add("$change_res_cmd /w=$min_width /h=$min_height /d=$id /f=$min_refesh") | Out-Null
            $reset_res_cmd_arr.Add("$change_res_cmd /w=$width /h=$height /d=$id /f=$refresh") | Out-Null
        } else { # Refresh rate below or equal to minimum
            $set_VR_res_cmd_arr.Add("$change_res_cmd /w=$min_width /h=$min_height /d=$id") | Out-Null
            $reset_res_cmd_arr.Add("$change_res_cmd /w=$width /h=$height /d=$id") | Out-Null
        }
    # Checking if monitor is high refresh rate 1080p panel
        ...
        $set_VR_res_cmd_arr.Add("$change_res_cmd /d=$id /f=$min_refresh") | Out-Null
        $reset_res_cmd_arr.Add("$change_res_cmd /d=$id /f=$refresh") | Out-Null
    }
}
